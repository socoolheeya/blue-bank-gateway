...
echo -e "  ${CYAN}●${NC} Account Service: ${ACCOUNT_INSTANCES}개 (포트: 8101-8199)"
echo -e "  ${GREEN}●${NC} Deposit Service: ${DEPOSIT_INSTANCES}개 (포트: 8201-8299)"
echo -e "  ${YELLOW}●${NC} Loan Service: ${LOAN_INSTANCES}개 (포트: 8301-8399)"
echo -e "  ${MAGENTA}●${NC} Card Service: ${CARD_INSTANCES}개 (포트: 8401-8499)"
echo ""
...
for service in "${services[@]}"; do
    ...
    if [ $count -gt 0 ]; then
        service_upper="$(echo ${service:0:1} | tr '[:lower:]' '[:upper:]')${service:1}"
        echo -e "  ${RED}▶${NC} ${service_upper} Service: ${count}개 컨테이너 중지..."
        ...
        service_upper="$(echo ${service:0:1} | tr '[:lower:]' '[:upper:]')${service:1}"
        echo -e "  ${GREEN}✓${NC} ${service_upper} Service: 정리할 컨테이너 없음"
    fi
done
...
if [ "$BUILD_FLAG" = true ]; then
    ...
    for service in "${services[@]}"; do
        service_upper="$(echo ${service:0:1} | tr '[:lower:]' '[:upper:]')${service:1}"
        echo -e "  ${BLUE}▶${NC} ${service_upper} Service 빌드..."
...
for service in "${services[@]}"; do
    ...
    # 첫 글자를 대문자로 변환 (macOS 호환)
    service_upper="$(echo ${service:0:1} | tr '[:lower:]' '[:upper:]')${service:1}"
    echo -e "  ${color}●${NC} ${service_upper}: ${count}개 실행 중"
done
echo -e "  ${BLUE}━━━━━━━━━━━━━━━━━━${NC}"
